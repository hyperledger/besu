...
public class CodeV0 implements Code {
  ...
  CodeV0(final Bytecode bytes) {
    ...
    this.validJumpDestinations = new Long[(bytes.size() >> 6) + 1];
  ...
  public boolean isJumpDestInvalid(final int jumpDestination) {
    ...
    final int pos = jumpDestination >> 6;
    if (validJumpDestinations[pos] == null) {
      validJumpDestinations[pos] = calculateJumpDest(pos);
    }
