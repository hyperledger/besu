...
public class MainnetMessageCallProcessor extends AbstractMessageProcessor {
  ...
  private void transferValue(final MessageFrame frame) {
    ...
    final var senderAccount = frame.getWorldState().getAccount(senderAddress);
    // The yellow paper explicitly states that if the recipient account doesn't exist at this
    // point, it is created.
    final var recipientAccount =
        frame.getWorldState().getOrCreateAccount(frame.getRecipientAddress());

    // TODO check failure to create

    final Wei initialSenderBalance = Wei.fromQuantity(senderAccount.getBalance());
    final Wei initialRecipientBalance = Wei.fromQuantity(recipientAccount.get().getBalance());
    final Wei updatedSenderBalance = initialSenderBalance.subtractExact(frame.getValue());
    final Wei updatedRecipientBalance = initialRecipientBalance.addExact(frame.getValue());
    frame.getWorldState().updateAccountBalance(frame.getSenderAddress(), updatedSenderBalance);
    frame
        .getWorldState()
        .updateAccountBalance(frame.getRecipientAddress(), updatedRecipientBalance);
