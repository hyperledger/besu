...
public class CodeV1 implements Code {
  ...
  private static final int MAX_STACK_HEIGHT = 1024;
  ...
  static String validateStack(final EOFLayout eofLayout) {
    return IntStream.range(0, eofLayout.getCodeSections().length)
        .mapToObj(i -> validateStack(i, eofLayout.getCodeSections()))
        .filter(Objects::nonNull)
        .findAny()
        .orElse(null);
  }
