...
public class SynchronizerOptions implements CLIOptions<SynchronizerConfiguration.Builder> {
  ...
  @CommandLine.Option(
      names = {"--Xsnapsync-synchronizer-pre-checkpoint-headers-only-enabled", SNAP_SYNC_SAVE_PRE_CHECKPOINT_HEADERS_ONLY_FLAG},
      paramLabel = "<Boolean>",
      arity = "0..1",
      description =
          "Enable snap sync downloader to save only headers (not block bodies) for blocks before the checkpoint. (default: ${DEFAULT-VALUE}) \"--Xsnapsync-synchronizer-pre-checkpoint-headers-only-enabled\" is deprecated and will be removed in a future release. Use \"" + SNAP_SYNC_SAVE_PRE_CHECKPOINT_HEADERS_ONLY_FLAG + "\" instead.")
