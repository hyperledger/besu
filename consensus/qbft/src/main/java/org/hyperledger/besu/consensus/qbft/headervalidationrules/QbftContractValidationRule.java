 ...
 */
public class QbftContractValidationRule implements AttachedBlockHeaderValidationRule {
  ...
  public boolean validate(
    ...
    try {
      ...
      if (!bftExtraData.getValidators().isEmpty()) {
        LOGGER.info(
            "Invalid block header: Validators expected to be empty when using validator smart contract got {}",
      ...
      if (bftExtraData.getVote().isPresent()) {
        LOGGER.info(
            "Invalid block header: Vote expected to be empty when using validator smart contract got {}",
