...
public class QbftController implements BftEventHandler {
  ...
  private final QbftFinalState qbftFinalState;
  ...
  public QbftController(
    ...
    this.qbftFinalState = qbftFinalState;
  ...
  private <P extends BftMessage<?>> void consumeMessage(
      final Message message, final P q, final Consumer<P> handleMessage) {
    LOG.trace("Received IBFT {} message", q.getClass().getSimpleName());
    ...
    if (q.getRoundIdentifier().getSequenceNumber() <= blockchain.getChainHeadBlockNumber()) {
      LOG.debug("Discarding a message which targets a height not above current chain height.");
      return;
    }

    if (processMessage(q, message)) {
      gossiper.send(message);
      handleMessage.accept(q);
    }
  }
  ...
  private boolean processMessage(final BftMessage<?> msg, final Message rawMsg) {
    ...
    if (isMsgForCurrentHeight(msgRoundIdentifier)) {
      return isMsgFromKnownValidator(msg) && qbftFinalState.isLocalNodeValidator();
  ...
  private boolean isMsgFromKnownValidator(final Authored msg) {
    return qbftFinalState.getValidators().contains(msg.getAuthor());
  }
