...
public class QbftProtocolManager implements ProtocolManager {
  private final BftEventQueue bftEventQueue;

  private final PeerConnectionTracker peers;

  /**
   * Constructor for the qbft protocol manager
   *
   * @param bftEventQueue Entry point into the ibft event processor
   * @param peers Used to track all connected IBFT peers.
   */
  public QbftProtocolManager(final BftEventQueue bftEventQueue, final PeerConnectionTracker peers) {
  ...
  public String getSupportedProtocol() {
    return QbftSubProtocol.get().getName();
  }

  @Override
  public List<Capability> getSupportedCapabilities() {
    return Arrays.asList(QbftSubProtocol.IBFV1);
  }
