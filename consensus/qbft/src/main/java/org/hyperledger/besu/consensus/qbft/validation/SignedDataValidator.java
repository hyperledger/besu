...
public class SignedDataValidator {
  ...
  public boolean validatePrepare(final SignedData<PreparePayload> msg) {
    ...
    return validateDigestMatchesBlock(msg.getPayload().getDigest(), msgType);
  }
  ...
  public boolean validateCommit(final SignedData<CommitPayload> msg) {
    ...
    final Hash proposedBlockDigest = proposal.get().getPayload().getProposedBlock().getHash();
  ...
  private boolean validateDigestMatchesBlock(final Hash digest, final String msgType) {
    final Hash proposedBlockDigest = proposal.get().getPayload().getProposedBlock().getHash();
  ...
  private boolean proposalMessagesAreIdentical(
    ...
    return right.getProposedBlock().getHash().equals(left.getProposedBlock().getHash())
