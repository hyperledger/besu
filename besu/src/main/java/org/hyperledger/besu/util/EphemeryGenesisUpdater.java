...
public class EphemeryGenesisUpdater {
   ...
   */
  public static GenesisFile updateGenesis(final Map<String, String> overrides)
    ...
    try {
      ...
      genesisFile = GenesisFile.fromResource(EPHEMERY.getGenesisFile());
      long genesisTimestamp = genesisFile.getTimestamp();
      Optional<BigInteger> genesisChainId = genesisFile.getConfigOptions().getChainId();
      ...
      if (currentTimestamp > (genesisTimestamp + PERIOD_IN_SECONDS)) {
        ...
        genesisFile = genesisFile.withOverrides(overrides);
      }
      return genesisFile.withOverrides(overrides);
    } catch (IOException e) {
