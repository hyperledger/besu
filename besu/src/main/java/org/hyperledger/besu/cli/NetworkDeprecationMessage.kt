...
object NetworkDeprecationMessage {
    /**
     ...
     */
    @JvmStatic
    fun generate(network: NetworkName): String {
        if (network.getDeprecationDate().isEmpty) {
            throw AssertionError("Deprecation date is not set. Cannot print a deprecation message")
        }
