...
public class BesuCommand implements DefaultCommandValues, Runnable {
  ...
  @Option(
      ...
      description =
          "Enable passing the revert reason back through TransactionReceipts (default:"
              + " ${DEFAULT-VALUE})")
  ...
  private void checkGoQuorumCompatibilityConfig() {
    if (genesisFile != null
      ...
      throw new ParameterException(
          this.commandLine,
          "--min-gas-price must be set to zero if GoQuorum compatibility is enabled in the genesis"
              + " config.");
    }
  }
