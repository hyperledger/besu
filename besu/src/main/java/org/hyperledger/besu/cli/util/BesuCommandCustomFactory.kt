 ...
 */(private val pluginVersionsProvider: PluginVersionsProvider) : CommandLine.IFactory {
    private val defaultFactory: CommandLine.IFactory = CommandLine.defaultFactory()

    @Throws(Exception::class)
    override fun <T> create(cls: Class<T>): T {
        if (CommandLine.IVersionProvider::class.java.isAssignableFrom(cls)) {
            return VersionProvider(pluginVersionsProvider) as T
        }
