...
public class MiningOptionsTest
    extends AbstractCLIOptionsTest<MiningParameters, MiningOptions> {


  @Test
  public void besuDoesNotStartInMiningModeIfCoinbaseNotSet() {
    internalTestFailure("Unable to mine without a valid coinbase. Either disable mining (remove --miner-enabled) or specify the beneficiary of mining (via --miner-coinbase <Address>)", "--miner-enabled");
...
//    parseCommand("--miner-enabled", "--miner-coinbase=" + coinbaseStr);
    internalTestSuccess(miningOpts -> {
    ...
    }, "--miner-enabled", "--miner-coinbase=" + coinbaseStr);
...
//
    ...
    }, "--miner-enabled", "--miner-coinbase=" + coinbaseStr, "--miner-stratum-enabled");
...
internalTestFailure("Unable to mine with Stratum if mining is disabled. Either disable Stratum mining (remove --miner-stratum-enabled) or specify mining is enabled (--miner-enabled)",
        "--network", "dev", "--miner-stratum-enabled");
...
//                    "Unable to mine with Stratum if mining is disabled. Either disable Stratum mining (remove --miner-stratum-enabled) or specify mining is enabled (--miner-enabled)");
  ...
  public void stratumMiningOptionsRequiresServiceToBeEnabledToml() throws IOException {
    ...
    internalTestFailure("Unable to mine with Stratum if mining is disabled. Either disable Stratum mining (remove --miner-stratum-enabled) or specify mining is enabled (--miner-enabled)",
            "--network", "dev", "--config-file", toml.toString());
...
//                    "Unable to mine with Stratum if mining is disabled. Either disable Stratum mining (remove --miner-stratum-enabled) or specify mining is enabled (--miner-enabled)");
  ...
  public void blockProducingOptionsWarnsMinerShouldBeEnabled() {
    ...
    internalTestSuccess(miningOpts -> verifyOptionsConstraintLoggerCall(
...
//            "0x1122334455667788990011223344556677889900112233445566778899001122");
    ...
    internalTestSuccess(miningOpts -> verify(mockLogger, atMost(0))
                    .warn(
...
//    assertThat(commandErrorOutput.toString(UTF_8)).isEmpty();
  ...
  public void blockProducingOptionsDoNotWarnWhenMergeEnabled() {
    ...
    final Address requestedCoinbase = Address.fromHexString("0000011111222223333344444");
    internalTestSuccess(miningOpt ->     verify(mockLogger, atMost(0))
            .warn(
                    stringArgumentCaptor.capture(),
                    stringArgumentCaptor.capture(),
                    stringArgumentCaptor.capture()),
...
//    assertThat(commandErrorOutput.toString(UTF_8)).isEmpty();
  ...
  public void minGasPriceRequiresMainOption() {
    internalTestSuccess(miningOpt -> verifyOptionsConstraintLoggerCall("--miner-enabled", "--min-gas-price"),
            "--min-gas-price", "0", "--network", "dev"
    );
//
  ...
  public void minGasPriceDoesNotRequireMainOptionWhenPoAQBFT() throws IOException {
    ...
    internalTestSuccess(miningOpt -> verify(mockLogger, atMost(0))
                    .warn(
                            ...
                            stringArgumentCaptor.capture(),
                            stringArgumentCaptor.capture()),
            "--genesis-file", genesisFileQBFT.toString(), "--min-gas-price", "0"
    );
  }
//
...
public void minGasPriceDoesNotRequireMainOptionWhenPoAIBFT2() throws IOException {
    ...
    internalTestSuccess(miningOpt ->     verify(mockLogger, atMost(0))
                    .warn(
                            ...
                            stringArgumentCaptor.capture(),
                            stringArgumentCaptor.capture()),
            "--genesis-file", genesisFileIBFT2.toString(), "--min-gas-price", "0"
    );
...
//    assertThat(commandErrorOutput.toString(UTF_8)).isEmpty();
  ...
  public void miningParametersAreCaptured() {
    ...
    final String extraDataString =
            "0x1122334455667788990011223344556677889900112233445566778899001122";
    internalTestSuccess(miningParams -> {
...
//            .isEqualTo(Bytes.fromHexString(extraDataString));
  ...
  public void targetGasLimitIsEnabledWhenSpecified() {
    internalTestSuccess(miningParams -> assertThat(
            miningParams.getDynamic().getTargetGasLimit().getAsLong())
            .isEqualTo(10_000_000L),         "--target-gas-limit=10000000");
...
//            .isEqualTo(GasLimitCalculator.constant());
  ...
  public void posBlockCreationMaxTimeDefaultValue() {
    internalTestSuccess(miningParams -> assertThat(miningParams.
            getPosBlockCreationMaxTime()).isEqualTo(DEFAULT_POS_BLOCK_CREATION_MAX_TIME));
//    parseCommand();
//    assertThat(getPosBlockCreationMaxTimeValue()).isEqualTo(DEFAULT_POS_BLOCK_CREATION_MAX_TIME);
  ...
  public void posBlockCreationMaxTimeOption() {
    internalTestSuccess(miningParams -> assertThat(miningParams.
            getPosBlockCreationMaxTime()).isEqualTo(7000L), "--Xpos-block-creation-max-time", "7000");
...
//  }
  ...
  public void posBlockCreationMaxTimeOutOfAllowedRange() {
    internalTestFailure("--Xpos-block-creation-max-time must be positive and ≤ 12000", "--Xpos-block-creation-max-time", "17000");
//
