...
abstract class AbstractSignatureAlgorithm implements SignatureAlgorithm {
  ...
  protected AbstractSignatureAlgorithm() {
    ...
    final X9ECParameters params = SECNamedCurves.getByName(getCurveName());
    ...
    final ECGenParameterSpec ecGenParameterSpec = new ECGenParameterSpec(getCurveName());
    try {
  ...
  public Bytes calculateECDHKeyAgreement(
    ...
    return SECPKeyUtil.toBytes(agreed, getPrivateKeyByteLength());
  }
  ...
  public SECPPrivateKey createPrivateKey(final BigInteger key) {
    return SECPPrivateKey.create(key, ALGORITHM, getPrivateKeyByteLength());
  }
  ...
  public SECPPublicKey createPublicKey(final BigInteger key) {
    return SECPPublicKey.create(key, ALGORITHM, getPublicKeyByteLength());
  }
  ...
  public SECPSignature createSignature(final BigInteger r, final BigInteger s, final byte recId) {
    return SECPSignature.create(r, s, recId, curveOrder, getPrivateKeyByteLength());
  }

  @Override
  public SECPSignature decodeSignature(final Bytes bytes) {
    return SECPSignature.decode(bytes, curveOrder, getPrivateKeyByteLength());
  }

  protected Optional<SECPPublicKey> recoverPublicKeyFromSignature(
      final Bytes dataHash, final SECPSignature signature, final BigInteger prime) {
    final BigInteger publicKeyBI =
        recoverFromSignature(
            signature.getRecId(), signature.getR(), signature.getS(), dataHash, prime);
    return Optional.of(SECPPublicKey.create(publicKeyBI, ALGORITHM, getPublicKeyByteLength()));
  }

  protected SECPSignature normaliseSignature(
    ...
    for (int i = 0; i < 4; i++) {
      final BigInteger k = recoverFromSignature(i, nativeR, s, dataHash, prime);
    ...
    return new SECPSignature(nativeR, s, (byte) recId, getPrivateKeyByteLength());
  }

  protected SECPSignature sign(final Bytes dataHash, final KeyPair keyPair, final Digest digest) {
    final ECDSASigner signer = new ECDSASigner(new HMacDSAKCalculator(digest));
