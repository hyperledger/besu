...
public class GoQuorumEnclave {
  ...
  public SendResponse sendSignedTransaction(final byte[] hash, final List<String> privateFor) {
    final GoQuorumSendSignedRequest request = new GoQuorumSendSignedRequest(hash, privateFor);
    return post(
        JSON,
        request,
        "/sendsignedtx",
