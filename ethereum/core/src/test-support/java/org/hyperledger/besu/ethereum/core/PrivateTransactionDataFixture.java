...
public class PrivateTransactionDataFixture {
  ...
  private static final SecureRandom secureRandom = SecureRandomProvider.createSecureRandom();
  ...
  public static final Address DEFAULT_SENDER = Address.fromHexString("0xfe3b557e8fb62b89f4916b721be55ceb828dbd73");
  ...
  public static final Bytes VALID_CONTRACT_DEPLOYMENT_PAYLOAD = Bytes.fromHexString(
  ...
  public static final Address VALID_CONTRACT_DEPLOYMENT_ADDRESS = Address.fromHexString("0x0bac79b78b9866ef11c989ad21a7fcf15f7a18d7");
  ...
  public static Transaction privacyMarkerTransactionOnChainAdd() {
    return privacyMarkerTransaction(Bytes.concatenate(VALID_BASE64_ENCLAVE_KEY, VALID_BASE64_ENCLAVE_KEY), Address.ONCHAIN_PRIVACY);
  }

  private static Transaction privacyMarkerTransaction(final Bytes transactionKey, final Address precompiledContractAddress) {
  ...
  public static PrivateTransaction privateContractDeploymentTransactionLegacy() {
    return new PrivateTransactionTestFixture().payload(VALID_CONTRACT_DEPLOYMENT_PAYLOAD).privateFor(Collections.singletonList(VALID_BASE64_ENCLAVE_KEY)).createTransaction(KEY_PAIR);
  }

  public static PrivateTransaction privateTransactionBesu() {
    return new PrivateTransactionTestFixture().privacyGroupId(VALID_BASE64_ENCLAVE_KEY).createTransaction(KEY_PAIR);
  }

  public static PrivateTransaction privateContractDeploymentTransactionBesu() {
    return new PrivateTransactionTestFixture().payload(VALID_CONTRACT_DEPLOYMENT_PAYLOAD).privacyGroupId(VALID_BASE64_ENCLAVE_KEY).createTransaction(KEY_PAIR);
  }

  public static ReceiveResponse generateReceiveResponse(final PrivateTransaction privateTransaction) {
    final BytesValueRLPOutput rlpOutput = new BytesValueRLPOutput();
    privateTransaction.writeTo(rlpOutput);
    return new ReceiveResponse(rlpOutput.encoded().toBase64String().getBytes(UTF_8), privateTransaction.getPrivacyGroupId().isPresent() ? privateTransaction.getPrivacyGroupId().get().toBase64String() : "", null);
  }

  public static ReceiveResponse generateVersionedReceiveResponse(final PrivateTransaction privateTransaction) {
  ...
  public static List<PrivateTransactionWithMetadata> generateAddBlobResponse(final PrivateTransaction privateTransaction, final Transaction markerTransaction) {
    final PrivateTransactionWithMetadata privateTransactionWithMetadata = new PrivateTransactionWithMetadata(privateTransaction, new PrivateTransactionMetadata(markerTransaction.getHash(), Hash.ZERO));
