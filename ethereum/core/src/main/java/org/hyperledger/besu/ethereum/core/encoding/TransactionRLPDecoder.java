...
public interface TransactionRLPDecoder {
  ...
  TransactionRLPDecoder ESCALATOR = escalatorDecoder();
  ...
  static TransactionRLPDecoder escalatorDecoder() {
    return input -> {
      input.enterList();
      final Transaction.Builder builder =
          Transaction.builder()
      ...
      final Bytes maybeStartPriceOrV = input.readBytes();
      final Bytes maybeStartBlockOrR = input.readBytes();
      final Bytes maybeMaxPriceOrS = input.readBytes();
      final BigInteger v, r, s;
      // if this is the end of the list we are processing a legacy transaction
      if (input.isEndOfCurrentList()) {
        v = maybeStartPriceOrV.toUnsignedBigInteger();
        r = maybeStartBlockOrR.toUnsignedBigInteger();
        s = maybeMaxPriceOrS.toUnsignedBigInteger();
      } else {
        // otherwise this is an Escalator transaction
        builder
            .escalatorStartPrice(Wei.of(maybeStartPriceOrV.toBigInteger()))
            .escalatorStartBlock(maybeStartBlockOrR.toLong())
            .escalatorMaxPrice(Wei.of(maybeMaxPriceOrS.toBigInteger()))
            .escalatorMaxBlock(input.readLongScalar());
        v = input.readBigIntegerScalar();
        r = input.readUInt256Scalar().toBytes().toUnsignedBigInteger();
        s = input.readUInt256Scalar().toBytes().toUnsignedBigInteger();
      }
      final byte recId;
