...
public class BonsaiSnapshotAccount implements MutableAccount {
  ...
  public UInt256 getStorageValue(final UInt256 key) {
    mutatedStorage.computeIfAbsent(key,
      k -> storageSlotFetcher.apply(address, key));

    return mutatedStorage.get(key)
  ...
  public NavigableMap<Bytes32, AccountStorageEntry> storageEntriesFrom(
	...
	throw new RuntimeException("Bonsai Snapshot Account does not currently support enumerating storage");
  }
  ...
  public Map<UInt256, UInt256> getUpdatedStorage() {
    ...
    .filter(e -> !e.getValue().get().isUnchanged())
      ...
      UInt256.fromBytes(entry.getKey()),
      entry.getValue().get().getUpdated()))
