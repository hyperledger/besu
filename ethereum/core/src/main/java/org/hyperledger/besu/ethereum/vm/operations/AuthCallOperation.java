...
public class AuthCallOperation extends AbstractCallOperation {
  ...
  public AuthCallOperation(final GasCalculator gasCalculator) {
    super(0xF7, "AUTHCALL", 8, 1, false, 1, gasCalculator);
  }
  ...
  public OperationResult execute(final MessageFrame frame, final EVM evm) {
    if (frame.getAuthorized() == null) {
      return new OperationResult(
          Optional.of(cost(frame)), Optional.of(ExceptionalHaltReason.UNAUTHORIZED_AUTHCALL));
    } else {
      final Gas cost = cost(frame);
      if (cost != null) {
        Gas callRemainingGas = frame.getRemainingGas().minus(cost);
        Gas sixtyThird = callRemainingGas.minus(callRemainingGas.dividedBy(64));
        if (gas(frame).compareTo(sixtyThird) == 1) { // we're sending more 63/64 of available gas
          return new OperationResult(
