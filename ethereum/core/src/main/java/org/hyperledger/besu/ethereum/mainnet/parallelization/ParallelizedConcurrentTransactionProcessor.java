...
public class ParallelizedConcurrentTransactionProcessor extends ParallelBlockTransactionProcessor {
  ...
  protected ParallelizedTransactionContext runTransaction(
    ...
    try {
      ...
      // Compose the block operation tracer (which may contain ExecutionMetricsTracer) with the
      // mining beneficiary tracer
      final OperationTracer composedTracer =
          blockProcessingContext != null
              ? TracerAggregator.combining(
                  blockProcessingContext.getOperationTracer(), miningBeneficiaryTracer)
