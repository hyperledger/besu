...
public class TrieLogPrunerTest {
  ...
  @Test
  public void trieLogs_pruned_in_reverse_order_within_pruning_limit() {
    Configurator.setLevel(LogManager.getLogger(TrieLogPruner.class).getName(), Level.TRACE);

    // Given

    // pruning window is below numBlocksToRetain and inside the pruningLimit offset.
    final long blocksToRetain = 3;
    final int pruningLimit = 2;
    TrieLogPruner trieLogPruner =
        new TrieLogPruner(rootWorldStateStorage, blockchain, blocksToRetain, pruningLimit);
