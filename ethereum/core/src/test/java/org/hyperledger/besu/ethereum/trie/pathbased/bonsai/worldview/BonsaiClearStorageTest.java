...
class BonsaiClearStorageTest {
   ...
   * of how many slots the contract has. Before the fix, any count above 256 would hang.
   */
  ...
  void frontierRootHashCompletesAfterClearingStorage(
    ...
    final MutableWorldState ws = createWorldStateWithStorage(slotCount);

    // Clear the contract's storage and commit so storageToClear is populated
    final WorldUpdater updater = ws.updater();
  ...
  private MutableWorldState createWorldStateWithStorage(final int slots) {
    final Blockchain blockchain = mock(Blockchain.class);
    final BonsaiWorldStateProvider archive =
        InMemoryKeyValueStorageProvider.createBonsaiInMemoryWorldStateArchive(blockchain);
    final MutableWorldState ws = archive.getWorldState();

    final WorldUpdater setup = ws.updater();
