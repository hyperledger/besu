...
public class BonsaiWorldStateKeyValueStorageTest {
    ...
    public void getAccountStateTrieNode_saveAndGetSpecialValues() {
        ...
        storage
                ...
                .putAccountStateTrieNode(
                        Bytes.EMPTY,  Hash.hash(MerklePatriciaTrie.EMPTY_TRIE_NODE), MerklePatriciaTrie.EMPTY_TRIE_NODE)
    ...
    public void getAccountStorageTrieNode_saveAndGetRegularValue() {
        ...
        final Bytes bytes = Bytes.fromHexString("0x123456");
        final BonsaiWorldStateKeyValueStorage storage = emptyStorage();
        storage.updater().putAccountStorageTrieNode(accountHash, location, Hash.hash(bytes), bytes).commit();

        assertThat(storage.getAccountStorageTrieNode(accountHash, location, Hash.hash(bytes))).contains(bytes);
    }
    ...
    public void isWorldStateAvailable_defaultIsFalse() {
        assertThat(emptyStorage().isWorldStateAvailable(UInt256.valueOf(1).toBytes(), Hash.EMPTY)).isFalse();
    }
    ...
    public void isWorldStateAvailable_StateAvailableByRootHash() {
        ...
        updater.getTrieBranchStorageTransaction()
                .put(WORLD_ROOT_HASH_KEY, rootHashKey.toArrayUnsafe());
        updater.commit();
        assertThat(storage.isWorldStateAvailable(Hash.wrap(Bytes32.wrap(rootHashKey)), Hash.EMPTY)).isTrue();
    }

    @Test
    public void isWorldStateAvailable_StateAvailableByBlockHash() {

        final BonsaiWorldStateKeyValueStorage storage = emptyStorage();
        final BonsaiWorldStateKeyValueStorage.Updater updater = storage.updater();
        final Bytes blockHash = Bytes32.fromHexString("0x01");
        updater.getTrieBranchStorageTransaction()
                .put(blockHash.toArrayUnsafe(), blockHash.toArrayUnsafe());
        updater.commit();
        assertThat(storage.isWorldStateAvailable(UInt256.valueOf(1).toBytes(),Hash.wrap(Bytes32.wrap(blockHash)))).isTrue();
    }
