...
public class PutVisitor<V> implements PathNodeVisitor<V> {
  ...
  public Node<V> visit(
    ...
    final Bytes newExtensionPath = extensionPath.slice(0, commonPathLength);
    ...
    final Optional<Bytes> leafLocation =
        Optional.of(Bytes.concatenate(location, newExtensionPath, Bytes.of(leafIndex)));
    final Optional<Bytes> updatedExtensionLocation =
        Optional.of(Bytes.concatenate(location, newExtensionPath, Bytes.of(extensionIndex)));

    final Node<V> updatedExtension =
        extensionNode.replacePath(extensionPath.slice(commonPathLength + 1));
    updatedExtension.setLocation(updatedExtensionLocation);
    final Node<V> leaf = nodeFactory.createLeaf(leafLocation, leafPath, value);
    final Node<V> branch =
  ...
  public Node<V> visit(final NullNode<V> nullNode, final Bytes location, final Bytes path) {
    return nodeFactory.createLeaf(Optional.of(location), path, value);
  }
}
