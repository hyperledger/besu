...
public class RlpxAgentTest {
  ...
  //    startAgent();
  //    final MockPeerConnection connection = (MockPeerConnection)
  // agent.connect(createPeer()).get();
  ...
  //
  //    // No peers should be disconnected - exempt connections are ignored when enforcing this
  // limit
  ...
  //      throws ExecutionException, InterruptedException {
  //    // Turn off autocomplete so that each connection is established (completed) after it has
  // been
  //    // successfully added to the internal connections set. This mimics async production
  // behavior.
  //    connectionInitializer.setAutocompleteConnections(false);
  //
  //    // Saturate connections
  //    startAgentWithLowerBoundPeers(1);
  //    final CompletableFuture<PeerConnection> existingConnectionFuture =
  // agent.connect(createPeer());
  ...
  //
  //    // Add existing peer
  //    final MockPeerConnection existingConnection = (MockPeerConnection)
  // agent.connect(peerA).get();
  //    assertThat(agent.getConnectionCount()).isEqualTo(1);
  //
  //    // Simulate incoming connection
