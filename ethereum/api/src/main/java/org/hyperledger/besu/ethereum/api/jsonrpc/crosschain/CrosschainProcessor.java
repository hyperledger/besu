...
public class CrosschainProcessor {
    ...
    public CrosschainProcessor(final SubordinateViewCoordinator subordinateViewCoordinator,
                               final TransactionSimulator transactionSimulator, final TransactionPool transactionPool) {
     ...
     */
    public ValidationResult<TransactionValidator.TransactionInvalidReason> addLocalTransaction(final CrosschainTransaction transaction) {
        // Get Subordinate View results.
        if (processSubordinates(transaction, false)) {
            return ValidationResult.invalid(TransactionValidator.TransactionInvalidReason.FAILED_SUBORDINATE_VIEW);
        }

        Optional<ValidationResult<TransactionValidator.TransactionInvalidReason>> executionError = trialExecution(transaction);
    ...
    private boolean processSubordinates(final CrosschainTransaction transaction, final boolean processSubbordianteTransactions) {
        for (CrosschainTransaction subordinateTransactionsAndView: transaction.getSubordinateTransactionsAndViews()) {
            if ((processSubbordianteTransactions && subordinateTransactionsAndView.getType().isSubordinateTransaction()) ||
                (!processSubbordianteTransactions && subordinateTransactionsAndView.getType().isSubordinateView())){

                String method = subordinateTransactionsAndView.getType().isSubordinateView() ? "eth_processSubordinateView" : "eth_sendRawCrosschainTransaction";
                ...
                // TODO check that the Subordiante View hash returned matches the submitted subordiante view.
     ...
     * @param subordinateTransaction
     * @return
     */
    public Optional<ValidationResult<TransactionValidator.TransactionInvalidReason>> trialExecution(final CrosschainTransaction subordinateTransaction) {
        ...
        Optional<TransactionSimulatorResult> result = this.transactionSimulator.processAtHead(subordinateTransaction);
        ...
        if (result.isPresent()) {
            ...
            // The transaction may have failed, but the transaction is valid. This could occur when a revert is thrown
        ...
        return Optional.of(ValidationResult.invalid(TransactionValidator.TransactionInvalidReason.UNKNOWN_FAILURE));
    }




    private static String post(final String address, final String method, final String params) throws Exception {
        URL url = new URL("http://" + address);
        URLConnection con = url.openConnection();
        HttpURLConnection http = (HttpURLConnection)con;
        http.setRequestMethod("POST");
        http.setDoOutput(true);
        byte[] out = ("{\"jsonrpc\":\"2.0\",\"method\":\"" + method + "\",\"params\":[\"" + params + "\"],\"id\":1}").getBytes(UTF_8);
