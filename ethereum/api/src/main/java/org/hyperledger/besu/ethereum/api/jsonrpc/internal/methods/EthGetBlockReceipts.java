...
public class EthGetBlockReceipts extends AbstractBlockParameterOrBlockHashMethod {
  ...
  public EthGetBlockReceipts(final BlockchainQueries blockchain) {
    this(Suppliers.ofInstance(blockchain));
  }

  public EthGetBlockReceipts(final Supplier<BlockchainQueries> blockchain) {
    super(blockchain);
  }
  ...
  private BlockReceiptsResult getBlockReceiptsResult(final Hash blockHash) {
    System.out.println(
        "Header for the requested block: "
            + getBlockchainQueries().blockByHash(blockHash).get().getHeader());
    return new BlockReceiptsResult(new ArrayList<>());
  }
}
