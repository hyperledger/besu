...
public class StructLogTest {
    ...
    public void constructorShouldInitializeFields() throws Exception {
        ...
        final String[] memory = new String[] {"0x01", "0x02", "0x0003", "0x04", "0x00000005", "0x000000"};
        final String[] stack = new String[] {"0x00000000000000", "0x01", "0x000002", "0x03"};
        final Map<String, String> storage = Map.of(
                "0x01", "0x2233333",
                "0x02", "0x4455667"
        );
        ...
        when(traceFrame.getMemory()).thenReturn(Optional.of(Arrays.stream(memory)
        ...
        when(traceFrame.getStack()).thenReturn(Optional.of(Arrays.stream(stack)
                .map(Bytes::fromHexString)  // Convert each string to Bytes
                .toArray(Bytes[]::new)));
        Map<UInt256, UInt256> storageMap = new HashMap<>();
        for (Map.Entry<String, String> entry : storage.entrySet()) {
            storageMap.put(UInt256.fromHexString(entry.getKey()), UInt256.fromHexString(entry.getValue()));
        }
        ...
        assertThat(structLog.memory()).isEqualTo(new String[] {"0x1", "0x2", "0x3", "0x4", "0x5", "0x0"});
        ...
        assertThat(structLog.storage()).isEqualTo(Map.of(
                "1", "2233333",
                "2", "4455667"
        ));
    ...
    public void shouldGenerateJsonCorrectly() throws Exception {
        ...
        final long gas = 100L;
        final OptionalLong gasCost = OptionalLong.of(10L);
        final String[] memory = new String[] {"0x01", "0x02", "0x0023", "0x04", "0x000000e5", "0x000000"};
        final String[] stack = new String[] {"0x00000000000000", "0x01", "0x000002", "0x03"};
        final Map<String, String> storage = Map.of(
                "0x01", "0x2233333",
                "0x02", "0x4455667"
    ...
    public void testToCompactHexWithLeadingZeros() {
        ...
        String result = StructLog.toCompactHex(bytes, true);
        assertEquals("0x1", result, "Expected '0x1' for the byte array 0x0001 with prefix (leading zeros removed)");
    }
