...
public class SyncBlockBodyTest {
    ...
    public static final DefaultProtocolSchedule DEFAULT_PROTOCOL_SCHEDULE = new DefaultProtocolSchedule(CHAIN_ID);

    @Test
    public void testGetBodyProvider() throws IOException {
        ...
        final SyncBlockBody syncBlockBody = SyncBlockBody.readWrappedBodyFrom(new BytesValueRLPInput(rlpBytes, false), false, protocolSchedule);
    ...
    public void testTxRootWdrRootOmmersHash() {
        ...
        block.getBody().writeWrappedBodyTo(bytesValueRLPOutput);
        final Bytes rlpBytes = bytesValueRLPOutput.encoded();
        final SyncBlockBody syncBlockBody = SyncBlockBody.readWrappedBodyFrom(new BytesValueRLPInput(rlpBytes, false), false, DEFAULT_PROTOCOL_SCHEDULE);

        assertThat(syncBlockBody.getTransactionsRoot()).isEqualTo(BodyValidation.transactionsRoot(block.getBody().getTransactions()));
        assertThat(syncBlockBody.getWithdrawalsRoot()).isEqualTo(BodyValidation.withdrawalsRoot(block.getBody().getWithdrawals().get()));
        assertThat(syncBlockBody.getOmmersHash()).isEqualTo(BodyValidation.ommersHash(block.getBody().getOmmers()));
    }

    @Test
    public void testEmptyWithEmptyWithdrawls() {
        final SyncBlockBody syncBlockBody = new SyncBlockBody(Bytes.EMPTY, Collections.emptyList(), Bytes.EMPTY, Collections.emptyList(), DEFAULT_PROTOCOL_SCHEDULE);

        final Keccak.Digest256 keccak = new Keccak.Digest256();

        assertThat(syncBlockBody.getTransactionCount()).isEqualTo(0);
        assertThat(syncBlockBody.getOmmersHash()).isEqualTo(Hash.wrap(Bytes32.wrap(keccak.digest(Bytes.EMPTY.toArray()))));
    ...
    public void testEmptyIsSameAsBlockBody2() {
        ...
        final SyncBlockBody syncBlockBody2 = SyncBlockBody.readWrappedBodyFrom(bytesValueRLPInput, true, DEFAULT_PROTOCOL_SCHEDULE);
