...
public class RetryingGetBytecodeFromPeerTaskTest
    extends AbstractMessageTaskTestBase<ByteCodesMessage.ByteCodes, Map<Bytes32, Bytes>> {

  private EthTask<Map<Bytes32, Bytes>> createTask() {
    return RetryingGetBytecodeFromPeerTask.forByteCode(
        ethContext,
        Collections.singletonList(Bytes32.ZERO),
        blockchain.getBlockHeader(1L).get(),
        metricsSystem);
  }
  ...
  public void checkEmptyResponseReducesReputation() {
    ...
    final EthTask<Map<Bytes32, Bytes>> task = createTask();
    final CompletableFuture<Map<Bytes32, Bytes>> future = task.run();
