...
public class RequestManagerTest {
  ...
  public void dispatchesMessagesReceivedAfterRegisteringCallback() throws Exception {
    for (final boolean supportsRequestId : List.of(true, false)) {
      ...
      // Send request
      final RequestManager.ResponseStream stream = requestManager.dispatchRequest(sender, null);
  ...
  public void dispatchesMessagesReceivedBeforeAndAfterRegisteringCallback() throws Exception {
    for (final boolean supportsRequestId : List.of(true, false)) {
      ...
      final RequestManager.ResponseStream stream = requestManager.dispatchRequest(sender, null);
      assertThat(sendCount.get()).isEqualTo(1);
      requestManager.dispatchRequest(sender, null);
  ...
  public void dispatchesMessagesToMultipleStreams() throws Exception {
    for (final boolean supportsRequestId : List.of(true, false)) {
      ...
      // Send request
      final RequestManager.ResponseStream streamA = requestManager.dispatchRequest(sender, null);
      final RequestManager.ResponseStream streamB = requestManager.dispatchRequest(sender, null);
      assertThat(sendCount.get()).isEqualTo(2);
