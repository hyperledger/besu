...
public class CompleteTaskStepTest {
  ...
  public void shouldMarkTaskAsFailedIfItDoesNotHaveData() {
    final StubTask task =
        new StubTask(NodeDataRequest.createAccountDataRequest(ROOT_HASH, Optional.empty(), true));
  ...
  private StubTask validTask() {
    ...
    final StubTask task =
        new StubTask(NodeDataRequest.createAccountDataRequest(hash, Optional.empty(), true));
