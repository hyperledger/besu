...
public class NewPooledTransactionHashesMessage68 extends AbstractNewPooledTransactionHashesMessage {
  ...
  NewPooledTransactionHashesMessage68(final Bytes rlp) {
    super(rlp);
  }

  public static AbstractNewPooledTransactionHashesMessage create(
      final List<Hash> pendingTransactions) {
    final BytesValueRLPOutput out = new BytesValueRLPOutput();
    final List<Hash> hashes = new ArrayList<>(pendingTransactions);
    out.startList();
    out.writeList(hashes, (h, w) -> w.writeBytes(h));
    out.endList();
    return new NewPooledTransactionHashesMessage68(out.encoded());
  }

  public static AbstractNewPooledTransactionHashesMessage readFrom(final MessageData message) {
    if (message instanceof NewPooledTransactionHashesMessage68) {
      return (NewPooledTransactionHashesMessage68) message;
    }
    ...
    return new NewPooledTransactionHashesMessage68(message.getData());
  }
