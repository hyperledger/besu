...
public class AccountRangeDataRequest extends SnapDataRequest {
  ...
  protected AccountRangeDataRequest(
    ...
    request =
        GetAccountRangeMessage.create(
            rootHash, startKeyHash, endKeyHash, BigInteger.valueOf(524288));
  }

  protected AccountRangeDataRequest(final Bytes data) {
    super(ACCOUNT_RANGE);
    request = new GetAccountRangeMessage(data);
  }
  ...
  protected void doPersist(final WorldStateStorage worldStateStorage, final Updater updater) {
    ...
    trie.commit(
        (location, nodeHash, value) -> {
          if (worldStateStorage.getAccountStateTrieNode(location, nodeHash).isEmpty()) {
            updater.putAccountStateTrieNode(location, nodeHash, value);
          }
        });
  }
  ...
  public Stream<SnapDataRequest> getChildRequests(final WorldStateStorage worldStateStorage) {
    ...
    if (responseData.accounts().isEmpty()) {
      ...
      findNewBeginElementInRange(getRange().endKeyHash(), responseData.accounts())
          .ifPresent(
              missingRightElement -> {
                childRequests.add(
      ...
      if (!missingBytecodes.isEmpty()) {
        childRequests.add(
            createBytecodeRequest(new ArrayDeque<>(accountsBytecodesToComplete), missingBytecodes));
      }
