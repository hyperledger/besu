...
class BeamSyncTargetManager<C> extends SyncTargetManager<C> {
  ...
  private final BlockHeader launchBlockHeader;

  public BeamSyncTargetManager(
      final SynchronizerConfiguration config,
      final ProtocolSchedule<C> protocolSchedule,
      final ProtocolContext<C> protocolContext,
      final EthContext ethContext,
      final MetricsSystem metricsSystem,
      final BlockHeader launchBlockHeader) {
  ...
  protected CompletableFuture<Optional<EthPeer>> selectBestAvailableSyncTarget() {
    ...
    if (!maybeBestPeer.isPresent()) {
      ...
      if (bestPeer.chainState().getEstimatedHeight() < launchBlockHeader.getNumber()) {
        LOG.info("No sync target with sufficient chain height, wait for peers.");
        return completedFuture(Optional.empty());
      } else {
        return confirmLaunchBlockHeader(bestPeer);
      }
    }
  }

  private CompletableFuture<Optional<EthPeer>> confirmLaunchBlockHeader(final EthPeer bestPeer) {
    final RetryingGetHeaderFromPeerByNumberTask task =
        RetryingGetHeaderFromPeerByNumberTask.forSingleNumber(
            protocolSchedule,
            ethContext,
            metricsSystem,
            launchBlockHeader.getNumber(),
    ...
    return ethContext
        ...
        .thenApply(
            result -> {
              if (peerHasDifferentLaunchBlock(result)) {
                LOG.warn(
                    "Best peer has wrong launch block (#{}) expecting {} but received {}.  Disconnect: {}",
                    launchBlockHeader.getNumber(),
                    launchBlockHeader.getHash(),
        ...
        .exceptionally(
            error -> {
              LOG.debug("Could not confirm best peer had launch block", error);
              return Optional.empty();
            });
  }

  private boolean peerHasDifferentLaunchBlock(final List<BlockHeader> result) {
    return result.size() != 1 || !result.get(0).equals(launchBlockHeader);
  }

  @Override
  public boolean shouldContinueDownloading() {
    return !protocolContext.getBlockchain().getChainHeadHash().equals(launchBlockHeader.getHash());
  }
}
