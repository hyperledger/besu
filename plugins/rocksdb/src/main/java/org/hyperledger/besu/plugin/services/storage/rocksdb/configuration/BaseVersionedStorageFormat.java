...
public enum BaseVersionedStorageFormat implements VersionedStorageFormat {
  ...
  /**
   * Current Bonsai archive proofs version, with blockchain variables in a dedicated column family,
   * in order to make BlobDB more effective
   */
  BONSAI_ARCHIVE_PROOFS_WITH_VARIABLES(DataStorageFormat.X_BONSAI_ARCHIVE_PROOFS, 1),
  /**
   * Current Bonsai archive proofs version, with receipts using compaction, in order to make
   * Receipts use less disk space
   */
  BONSAI_ARCHIVE_PROOFS_WITH_RECEIPT_COMPACTION(DataStorageFormat.X_BONSAI_ARCHIVE_PROOFS, 2);
